...
sim %>% 
  ggplot(aes(Distance, fill = Sem)) +
  geom_density(alpha = 0.4) +
  theme_minimal() +
  scale_fill_manual(values=c("yellow", "purple")) +
  labs(y = "Density", fill = "Semantic categories")

# NOTES AFTER

# Distribution: Normalish but bound to zero and some maximal distance
```
...
# How to determine these numbers? Can I look at the data or am I supposed to take my best guess?
...
#| label: spec_priors
#| 
my_priors <- c(prior(normal(78, 40), class = 'Intercept'), # the mean and sd of distance, makes sense?
                 prior(normal(20, 10), class = "b"), #
                 prior(normal(0, 10), class = 'sigma')) # what to put here? I'll just keep it the same
                 
# This is from wide_normal
...
# The posteriors samples are less peaky - the dark blue line is higher and narrower

# But I don't know how to interpret this - how much difference is a "problem"?

# probability distribution - read!!
```
...
# better when distance is turned into square root
```
