...
incomplete_model <- glmer(chosen_voicing ~ correct_voicing +
                        (1 + correct_voicing | listener) +
                          # random intercept: different listeners may have different starting points (mean??) for correctly identifying the items
                          # random slope: different listeners may also show different effects of ??correct_voicing??
                        (1 + correct_voicing | item_pair),
                        # random intercept: different item pairs may differ in how difficult they are to identify correctly
                        # random slope: different item pairs may differ in how they affect different speakers??
                        
                        # should I also include voice as a random effect?
                    data = incomplete, family = "binomial", # why binomila? couldn't make it work otherwise, also why glmer and not lmer?
                      )

# I don't fully know why I am doing what I'm doing here, I would have expected to use the ACC column in some way
```
...
summary(incomplete_model)

# Interpretation: 
# Intercept: When correct_voicingvoiceless is 0, estimated chosen_voicing is -0.05933 ??___??
# For each additional unit of correct_voicingvoiceless, estimated chosen_voicing increases by 0.48 ??___??

# null model
incomplete_model_2 <- glmer(chosen_voicing ~ correct_voicing +
                        (1 + correct_voicing | listener) +
                        (1 + correct_voicing | item_pair),
                        #REML = FALSE, # didn't work with this
                      data = incomplete,
                      family = "binomial"
                      )

# null model
incomplete_model_2_null <- glmer(chosen_voicing ~ 1 +
                        (1 + correct_voicing | listener) +
                        (1 + correct_voicing | item_pair),
                        #REML = FALSE, # same
                      data = incomplete,
...
anova(incomplete_model_2, incomplete_model_2_null)

# Interpretation:
# This effect is significant at an alpha level of 0.05, i.e. when assuming the null-hypothesis (i.e. the ??effect?? of correct_voicing is zero), the observed or more extreme test statistics are sufficiently improbable (p < 0.001) such that we can reject the null hypothesis.
```
...
plot_pred <- predict_response(incomplete_model_2, terms = c("correct_voicing"))
plot_pred
plot(plot_pred, color = "blue")
